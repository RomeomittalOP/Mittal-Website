...
export default function Jayden() {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    {
      ...
      from: "jayden",
      text:
        "Hi 👋 I'm JAYDEN, your AI Website Consultant. Tell me about your business and I'll recommend the best website solution, features, timeline and budget range.",
    },
  ]);
  ...
  /* ---------- helpers to add messages with typing delay ---------- */

  const pushUser = (text: string) => setMessages((m) => [...m, { id: id(), from: "user", text }]);
  ...
  const startWith = async (businessType: BusinessType, label: string) => {
    ...
    await sayWithTyping(550, {
      text:
        "Got it. What's your main goal with this website? Pick the closest one — we'll tailor everything around it.",
    });
  };
  ...
  const chooseHasWebsite = async (has: boolean) => {
    ...
    await sayWithTyping(500, {
      text:
        "Which features do you want? Pick all that apply — you can skip if you're not sure yet.",
    });
  };
  ...
  const toggleFeature = (f: Feature) => {
    setAnswers((a) => {
      ...
      return { ...a, features: cur.includes(f) ? cur.filter((x) => x !== f) : [...cur, f] };
    });
  };
  ...
  const chooseTimeline = async (t: Timeline, label: string) => {
    ...
    setStep("ask-budget");
    await sayWithTyping(500, {
      text: "And approximately what's your budget? This just helps me recommend the right tier.",
    });
  };
  ...
  const chooseBudget = async (b: Budget, label: string) => {
    ...
    await sayWithTyping(700, {
      text:
        "Shall I share your details with the team? They'll reach out within 24 hours to start your project.",
    });
        ...
        node: (
          <div className="flex items-start gap-2 rounded-xl border border-white/15 bg-white/[0.05] p-3">
            ...
            <div className="text-sm leading-relaxed text-white/85">
              <span className="font-semibold">All set, {name.split(" ")[0]}!</span> Our team will
              reach out within 24 hours. Anything else I can answer?
            </div>
          </div>
        ),
      });
  ...
  const sendFree = async (e?: React.FormEvent) => {
    ...
    if (ans) {
      ...
      pushJayden({
        text:
          "Good question. Easiest way is to chat with the team directly — they'll have an exact answer in minutes. Want me to share your details?",
      });
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 right-0 left-0 z-[82] mx-auto flex h-[88vh] w-full flex-col overflow-hidden rounded-t-3xl border border-white/12 bg-[#0d0d10]/95 shadow-[0_30px_80px_-10px_rgba(0,0,0,0.8)] backdrop-blur-xl sm:bottom-5 sm:left-auto sm:right-5 sm:h-[640px] sm:max-h-[80vh] sm:w-[400px] sm:rounded-3xl"
            >
              {/* glow accent */}
              ...
              <div className="relative flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div className="leading-tight">
                    ...
                    <div className="text-[11px] text-white/50">Your AI Website Consultant</div>
                  </div>
                </div>
                  ...
                  <form onSubmit={submitLead} className="space-y-2 pt-1">
                    ...
                    {submitState === "error" && (
                      <p className="text-xs text-red-400">
                        Couldn&apos;t send — please WhatsApp us at {BRAND.phoneDisplay}.
                      </p>
                    )}
                    <button
                      type="submit"
              ...
              {/* Composer (always available for free-form questions) */}
              <form
                onSubmit={sendFree}
